...
* Si vous souhaitez créer une sauvegarde basée sur des fichiers alors qu'une ou plusieurs bases de données de tenant sont en panne, définissez le paramètre ALLOW_FILE_BASED_BACKUP_IFINA_TENANTS_PRESIDENT sur *YES* dans le fichier de propriétés HANA à l'aide de l'applet de commande set-SmConfigSettings.
+
...
. Sur la page *Resource*, filtrez les ressources de la liste déroulante *View* en fonction du type de ressource.
+
...
. Sur la page *Resource*, sélectionnez *utiliser le format de nom personnalisé pour la copie Snapshot*, puis entrez un format de nom personnalisé à utiliser pour le nom de la copie Snapshot.
+
...
. Sur la page *Paramètres d'application*, procédez comme suit :
+
...
. Sur la page *Policies*, effectuez les opérations suivantes :
+
