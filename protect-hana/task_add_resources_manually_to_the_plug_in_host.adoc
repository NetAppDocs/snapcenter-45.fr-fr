...
. Sur la page *Ressources*, cliquez sur *Ajouter base de données SAP HANA*.
. Sur la page *fournir les détails de ressource*, effectuez les opérations suivantes :
+
|===
...
Clés de stockage sécurisé HDB
 a| 
Entrez la clé pour vous connecter au système SAP HANA. La clé contient les informations de connexion à la base de données.
...
|===
. Sur la page *Entitetour Storage Footprint*, sélectionnez un système de stockage et choisissez un ou plusieurs volumes, LUN et qtrees, puis cliquez sur *Save*.
+
...
Vous ne pouvez pas modifier ce qui suit si la ressource SAP HANA est associée à des sauvegardes :
