...
. Dans la page *Restore Scope*, sélectionnez *Complete Resource* pour restaurer les volumes de données configurés de la base de données SAP HANA.
+
...
. Sur la page *portée de récupération*, sélectionnez l'une des options suivantes :
+
|===
...
Ne pas récupérer
 a| 
Choisissez *pas de récupération*. Vous devez effectuer l'opération de restauration manuellement à partir du studio HANA.
