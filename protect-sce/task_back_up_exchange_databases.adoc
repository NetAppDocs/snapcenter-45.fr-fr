...
. Sur la page Ressources, sélectionnez *Database* ou *Database Availability Group* dans la liste *View*.
+
Sur la page Ressources, le image:../media/not_supported_icon.png[""] L'icône indique que la base de données se trouve sur un système de stockage non NetApp.
...
. Si vous souhaitez spécifier un nom de copie snapshot personnalisé, sur la page Ressources, cochez la case *utiliser le format de nom personnalisé pour la copie Snapshot*, puis entrez un format de nom personnalisé à utiliser pour le nom de la copie Snapshot.
+
