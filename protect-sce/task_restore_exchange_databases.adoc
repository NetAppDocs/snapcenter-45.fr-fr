...
. Sur la page Ressources, sélectionnez *base de données* dans la liste Affichage.
...
. Sur la page Options, sélectionnez l'une des options de sauvegarde des journaux suivantes :
+
|===
| Option | Description 
...
. Sur la page script, entrez le chemin d'accès et les arguments du prescripteur ou du PostScript qui doivent être exécutés avant ou après l'opération de restauration, respectivement.
+
