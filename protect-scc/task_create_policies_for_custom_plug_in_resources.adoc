...
** Dans la section *Paramètres de sauvegarde personnalisés*, indiquez tous les paramètres de sauvegarde spécifiques qui doivent être transmis au plug-in au format clé-valeur. Vous pouvez fournir plusieurs valeurs de clé à transmettre au plug-in.


. Sur la page *Retention*, spécifiez les paramètres de conservation pour le type de sauvegarde et le type de planification sélectionné dans la page Type de sauvegarde :
+
|===
...
|===
. Sur la page *Replication*, spécifiez les paramètres de réplication :
+
|===
