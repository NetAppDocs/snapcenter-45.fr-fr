...
. Sur la page Nom, effectuez l'une des opérations suivantes :
+
|===
...
. Sur la page opérations postales, *récupérer la base de données* et *jusqu'à Annuler* sont sélectionnés par défaut pour effectuer la récupération de la base de données clonée.
+
