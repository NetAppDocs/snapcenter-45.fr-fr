...
Les configurations prises en charge par le plug-in SnapCenter pour base de données Oracle sont répertoriées dans le https://mysupport.netapp.com/matrix/imt.jsp?components=100747;&solution=1257&isHWU&src=IMT["Matrice d'interopérabilité NetApp"^].
...
. Sur la page Ressources, assurez-vous que la base de données à importer est découverte et affichée.
+
